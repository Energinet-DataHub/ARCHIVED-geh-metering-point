resource "azurerm_monitor_action_group" "metering_point" {
  ...
  email_receiver {
    name                    = "Alerts-Development-${lower(var.environment_instance)}"
    email_address           = "214a2834.energinet.onmicrosoft.com@emea.teams.ms"
    use_common_alert_schema = true
  }
}


resource "azurerm_monitor_scheduled_query_rules_alert" "metering_point_outbox" {
