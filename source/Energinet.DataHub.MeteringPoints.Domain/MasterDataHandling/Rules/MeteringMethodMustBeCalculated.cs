...
{
    public class MeteringMethodMustBeCalculated : IBusinessRule
    {
        public MeteringMethodMustBeCalculated(MasterData masterData)
        {
            if (masterData == null) throw new ArgumentNullException(nameof(masterData));
            IsBroken = !(masterData.MeteringConfiguration.Method == MeteringMethod.Calculated);
        }
