...
{
    public class RejectMessageFactoryTests : TestBase
    {
        private MarketRoleParticipant? _sender;
        private MarketRoleParticipant? _receiver;
        private MarketActivityRecordWithReasons? _marketActivityRecord;
        ...
        {
            SetupVariables();
            var sut = RejectMessageFactory.CreateMeteringPoint(
                _sender!,
                _receiver!,
        ...
        {
            SetupVariables();
            var sut = RejectMessageFactory.UpdateMeteringPoint(
                _sender!,
                _receiver!,
                Instant.FromUtc(_now.Year, _now.Month, _now.Day, _now.Hour, _now.Minute),
                _marketActivityRecord!);
        ...
        {
            SetupVariables();
            var sut = RejectMessageFactory.ConnectMeteringPoint(
                _sender!,
                _receiver!,
                Instant.FromUtc(_now.Year, _now.Month, _now.Day, _now.Hour, _now.Minute),
                _marketActivityRecord!);
        ...
        {
            SetupVariables();
            var sut = RejectMessageFactory.ConnectionStatusMeteringPoint(
                _sender!,
                _receiver!,
                Instant.FromUtc(_now.Year, _now.Month, _now.Day, _now.Hour, _now.Minute),
                _marketActivityRecord!);
        ...
        {
            SetupVariables();
            var sut = RejectMessageFactory.RequestCloseDown(
                _sender!,
                _receiver!,
                Instant.FromUtc(_now.Year, _now.Month, _now.Day, _now.Hour, _now.Minute),
                _marketActivityRecord!);
        ...
        private void SetupVariables()
        {
            ...
            var reasons = new ReadOnlyCollectionBuilder<Reason>(new Collection<Reason>
            {
                new Reason("D99", "Test reason"),
            });
            ...
            _marketActivityRecord = new MarketActivityRecordWithReasons(
                Guid.NewGuid().ToString(),
                Guid.NewGuid().ToString(),
                Guid.NewGuid().ToString(),
                reasons.ToReadOnlyCollection());
        }
    }
}
