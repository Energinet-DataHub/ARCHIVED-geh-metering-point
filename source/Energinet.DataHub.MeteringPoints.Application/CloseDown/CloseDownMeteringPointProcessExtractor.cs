...
{
    public class CloseDownMeteringPointProcessExtractor : ProcessExtractor<CloseDownMeteringPoint>
    {
        public CloseDownMeteringPointProcessExtractor(IActorContext actorContext, ISystemDateTimeProvider dateTimeProvider)
            : base(actorContext, dateTimeProvider)
        {
        }

        protected override string ProcessName => "BRS-999";

        public override ProcessDetail GetProcessDetails(CloseDownMeteringPoint request)
        {
            if (request == null) throw new ArgumentNullException(nameof(request));

            return new ProcessDetail(
                "RequestCloseDownMeteringPoint",
                CurrentActor,
                DataHub,
                UtcNow,
                UtcNow,
        ...
        public override ProcessDetail GetProcessDetails(BusinessProcessResult result)
        {
            ...
            var name = result.Success
                ? "ConfirmCloseDownMeteringPoint"
                : "RejectCloseDownMeteringPoint";
        ...
        protected override string GetGsrn(CloseDownMeteringPoint request) => request?.GsrnNumber
